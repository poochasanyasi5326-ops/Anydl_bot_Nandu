...
# 🚀 TURBO TRACKERS (Injects into magnets to find seeds faster)
TRACKER_LIST = [
...
# --- 2. DASHBOARD FUNCTION (Defined before it's called) ---
...
async def show_dashboard(client, chat_id, msg_id, user_id):
    ...
    if t["is_youtube"]:
        text = f"📺 **YouTube**\n`{name}`"
        for f in t["yt_resolutions"]:
            buttons.append([InlineKeyboardButton(f"🎬 {f['res']} ({f['size']})", callback_data=f"yt_set_{f['height']}")])
...
async def process_task(client, status_msg, u_id):
    ...
    try:
        if t["is_youtube"]:
            f_path = await asyncio.to_thread(run_ytdlp, url, mode, d_path)
        elif t["is_torrent"]:
            if "magnet:?" in url:
        ...
        if not f_path or not os.path.exists(f_path):
            await status_msg.edit("❌ Error: File not found.")
            return
